module.exports.Graph = function(low, high) {
	...
	this.joinGraph = function(other_graph) {
		...
		// Otherwise, add them!
		for (var x in other_graphs.nodes) {
			this.nodes.push(other_graphs.nodes[x]);
		}
		
		for (var x in other_graphs.edges) {
			this.edges.push(other_graphs.edges[x]);
		}
